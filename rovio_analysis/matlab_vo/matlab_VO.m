...
images = image_bag.cam0_bag;
% ROVIO values.
K = [458.654, 0.0, 367.215; 0.0, 457.296, 248.375; 0.0, 0.0, 1.0]'; % Why the transpose in the example?
radial_distortion = [-0.28340811, 0.07395907, 0.0]; % k1-k3
tangential_distortion = [0.00019359, 1.76187114e-05]; %p1,p2
% Demo values
...
image_msg = readMessages(images, starting_image);
I_gray = readImage(image_msg{1});
player = vision.VideoPlayer('Position', [20, 400, 650, 510]);
step(player, I_gray);

% Set camera params after we know image size.
cameraParams = cameraParameters('IntrinsicMatrix', K, 'TangentialDistortion',tangential_distortion, ...
                                'RadialDistortion', radial_distortion,'ImageSize', [size(I_gray,1), size(I_gray,2)]);
...
viewId = 2;
image_msg = readMessages(images, viewId+starting_image);
I_gray = readImage(image_msg{1});
step(player, I_gray);

% Convert to gray scale and undistort.
I = undistortImage(I_gray, cameraParams);
...
for viewId = 16:images.NumMessages
    % Read and display the next image
    image_msg = readMessages(images, viewId+starting_image);
    I_gray = readImage(image_msg{1});
    step(player, I_gray);
